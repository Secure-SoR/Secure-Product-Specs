...
**Use when:** [implementation-guide-phase-3-dc.md](../specs/implementation-guide-phase-3-dc.md) Step 3.4 — embed geopolitical, climate, and cyber SitDeck widgets on the **single data centre property** overview; anchor to `properties.latitude` / `properties.longitude`; show only when SitDeck is connected and coordinates exist.

**Prerequisites:** [LOVABLE-PROMPT-SITDECK-INTEGRATIONS-ACCOUNT-SETTINGS.md](LOVABLE-PROMPT-SITDECK-INTEGRATIONS-ACCOUNT-SETTINGS.md) (connector + “connected” signal), [LOVABLE-PROMPT-PROPERTY-LAT-LNG.md](LOVABLE-PROMPT-PROPERTY-LAT-LNG.md) (lat/lng on property).

**Duplicate for IDE links:** [../specs/LOVABLE-PROMPT-SITDECK-EMBED-DC-PROPERTY-VIEW.md](../specs/LOVABLE-PROMPT-SITDECK-EMBED-DC-PROPERTY-VIEW.md) — keep both in sync.

**Spec:** [secure-dc-spec-v2.md](../specs/secure-dc-spec-v2.md) §6 (SitDeck OSINT); canonical route for the view: `/dashboards/data-centre/:propertyId`.
...
- Replace any placeholder embed builders with SitDeck’s real embed contract once you have it from SitDeck.
- Test: connected + coords → three panels load; disconnect or clear coords → section hides or shows guidance only.
- End-to-end embed token path should be reviewed for security (no long-lived secrets in the browser).
